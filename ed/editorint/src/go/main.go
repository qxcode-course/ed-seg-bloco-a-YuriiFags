...
func (e *Editor) KeyBackspace() {
	...
	if e.line != e.lines.Front() {
		...
		posFinal := linhaAnterior.Value.End()
		posInicial := e.line.Value.Front()

		for posInicial != e.line.Value.End() {
			proximo := posInicial.Next()
			linhaAnterior.Value.Insert(posFinal, posInicial.Value)
			e.line.Value.Erase(posInicial)
			posInicial = proximo
		}
...
func (e *Editor) KeyDelete() {
	...
	if e.line != e.lines.Back() {
        proximaLinha := e.line.Next()
       
        dest := e.line.Value.End()
        caractereAtual := proximaLinha.Value.Front()
        for caractereAtual != proximaLinha.Value.End() {
            nextcaractereAtual := caractereAtual.Next()
            e.line.Value.Insert(dest, caractereAtual.Value)
            proximaLinha.Value.Erase(caractereAtual)
            caractereAtual = nextcaractereAtual
        }
        e.lines.Erase(proximaLinha)
